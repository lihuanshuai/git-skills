...
def _require_confirmed_new_files(confirmed_files: Sequence[str]) -> None:
    staged = set(_staged_new_files())
    confirmed = set(confirmed_files)
    if staged != confirmed:
        missing = sorted(staged - confirmed)
        extra = sorted(confirmed - staged)
        raise StepError(
            "staged new files require explicit user confirmation; "
            f"unconfirmed={missing!r}, not_staged={extra!r}; "
            "pass --confirmed-new-file for each confirmed path",
            2,
        )
...
def run(args: argparse.Namespace) -> int:
    ...
    _require_confirmed_new_files(args.confirmed_new_file)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="PATH",
        help="repo-relative staged new file explicitly confirmed by the user; repeat per file",
    )
    parser.add_argument(
